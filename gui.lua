...
do
    local btnW     = math.floor((CONT_W-14)/3)
    local modeNames = {"Orbit","Cursor","Tornado"}
    local modeIcons = {"◆","◎","🌀"}
    local ndsMBtns  = {}
...
infoLbl(p2, "Сила притяжения (100-5000)", y2); y2 += 14
mkSlider(p2, y2, 100, 5000, ndsAttractionStr, false, function(v) ndsAttractionStr=v end); y2 += 16
infoLbl(p2, "Высота орбиты (-50 до +50)", y2); y2 += 14
mkSlider(p2, y2, -50, 50, ndsHeightOffset, false, function(v) ndsHeightOffset=v end); y2 += 16
infoLbl(p2, "Высота торнадо (10-500)", y2); y2 += 14
mkSlider(p2, y2, 10, 500, ndsTornadoHeight, false, function(v) ndsTornadoHeight=v end); y2 += 16
