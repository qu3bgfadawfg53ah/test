...
function AerialLocomotionController:_initiateAerialLocomotion()
    ...
    -- БАГ FIX: _bodyPositionInstance не очищался при повторном вызове _initiateAerialLocomotion
    if self._bodyPositionInstance then
        pcall(function() self._bodyPositionInstance:Destroy() end)
        self._bodyPositionInstance = nil
    end
    ...
    bodyVelocityObject.MaxForce = Vector3.new(math.huge, math.huge, math.huge)  -- Максимальная сила - ничто не сдвинет
    bodyVelocityObject.P = 9000  -- Увеличена жёсткость
    ...
    -- Создание BodyPosition (жёсткая фиксация позиции)
    local bodyPositionObject = Instance.new("BodyPosition")
    bodyPositionObject.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
    bodyPositionObject.P = 50000  -- Очень жёсткая фиксация
    bodyPositionObject.D = 5000
    bodyPositionObject.Position = humanoidRootPart.Position
    bodyPositionObject.Parent = humanoidRootPart
    self._bodyPositionInstance = bodyPositionObject
    ...
    self._physicsCycleConnection = RunService.Heartbeat:Connect(function(deltaTime)
        ...
        if directionVector.Magnitude > 0.001 then
            -- Активное движение - используем BodyVelocity
            targetVelocityVector = directionVector.Unit 
                * self._aerialVelocityMagnitude * speedMultiplier
            
            -- Отключаем BodyPosition во время движения
            if self._bodyPositionInstance then
                self._bodyPositionInstance.MaxForce = Vector3.zero
            end
        else
            -- Остановка - жёстко фиксируем позицию через BodyPosition
            targetVelocityVector = Vector3.zero
            
            -- Включаем BodyPosition для жёсткой фиксации
            if self._bodyPositionInstance then
                self._bodyPositionInstance.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
                self._bodyPositionInstance.Position = humanoidRootPart.Position
            end
        end
...
function AerialLocomotionController:_terminateAerialLocomotion()
    ...
    if self._bodyPositionInstance then
        self._bodyPositionInstance:Destroy()
        self._bodyPositionInstance = nil
    end
...
function PlayerKinematicsControlSystem:_initializeCharacterMonitoring()
    LocalPlayer.CharacterAdded:Connect(function(characterModel)
        -- БАГ FIX: полный сброс ВСЕХ физических объектов при респауне
        -- _bodyPositionInstance ранее не нилился — stale-ссылка на удалённый объект
        if self._aerialController._bodyPositionInstance then
            pcall(function() self._aerialController._bodyPositionInstance:Destroy() end)
            self._aerialController._bodyPositionInstance = nil
        end
        ...
        -- БАГ FIX: запоминаем флаг и СБРАСЫВАЕМ его немедленно.
        -- Если CharacterAdded отключил Heartbeat ДО его срабатывания,
        -- _aerialLocomotionActive оставался true. Тогда _initiateAerialLocomotion()
        -- вызывался до приземления — HRP.Position фиксировало воздушные координаты,
        -- BodyPosition с P=50000 тянул персонажа туда с огромной силой.
        local wasFlying = self._aerialController._aerialLocomotionActive
        self._aerialController._aerialLocomotionActive = false

        -- Привязка обработчика прыжков
        self._jumpModifier:_attachJumpHandlerToCharacter(characterModel)

        -- БАГ FIX: ждём приземления, а не фиксированных 0.4s.
        -- За 0.4s персонаж ещё падает — HRP.Position некорректен.
        local hrp = characterModel:WaitForChild("HumanoidRootPart", 5)
        if hrp then
            local hum = characterModel:FindFirstChildOfClass("Humanoid")
            if hum then
                local grounded = false
                local groundConn
                groundConn = hum.StateChanged:Connect(function(_, newState)
                ...
                -- Таймаут 2s: respawn над void или быстрый respawn без приземления
                local elapsed = 0
                repeat
                    task.wait(0.05)
                    elapsed = elapsed + 0.05
                until grounded or elapsed >= 2.0
                if groundConn then groundConn:Disconnect() end
            else
                task.wait(0.5)
            end
        ...
        if wasFlying then
            self._aerialController._aerialLocomotionActive = true
...
-- ═══════════════════════════════════════════════════════════════════
-- [МЕТОД] _reinitializeCharacterReference
-- Вызывается из main_loop.lua после респауна (БАГ FIX #32)
-- Ранее метод отсутствовал — pcall в main_loop.lua молча глотал ошибку,
-- переинициализация не происходила вообще.
-- ═══════════════════════════════════════════════════════════════════
function PlayerKinematicsControlSystem:_reinitializeCharacterReference()
    local characterModel = LocalPlayer.Character
    if not characterModel then
        addLog("PKCS ▸ _reinitializeCharacterReference: нет персонажа, пропуск")
        return
    end

    -- Обновляем кэш NoClip-частей
    self._collisionProcessor:_refreshCharacterPartsCache()

    -- Переподключаем Jump-обработчик к новому персонажу
    self._jumpModifier:_attachJumpHandlerToCharacter(characterModel)

    -- Восстанавливаем скорость если была активна
    if self._velocityEngine._velocityEnhancementActive then
        local hum = characterModel:FindFirstChildOfClass("Humanoid")
        if hum then
            hum.WalkSpeed = self._velocityEngine._targetVelocityMagnitude
        end
    end

    addLog("PKCS ▸ Character reference reinitialized")
end
