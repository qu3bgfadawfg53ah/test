...
function doTeleport(tgtPos, reason)
    ...
    local ok = pcall(function()
        -- Проверяем активен ли флай (проверяем через глобальные переменные)
        local flyIsActive = flyEnabled or (flyBodyVelocity and flyBodyVelocity.Parent)
        
        if flyIsActive then
            -- Флай активен - только телепортируем позицию, не трогаем физику
            hrp.CFrame = CFrame.new(tgtPos)
        else
            -- Флай не активен - обычный телепорт с BodyVelocity
